...
// isLoaded(auth) true means request done(might be completed or error) , isLoaded false means request is pending 
// isEmpty(auth) true means firebase.auth has user and false means it has no user 
  ...
  const onSubmit= async()=>{    
     console.log("On Submit:" , props.auth) ;
    ...
    {/* To save from multiple request */}
      {!isLoaded(props.auth)?<></>:
      <>
        {props.authMine.loading?<h4 style={{marginTop:'10%',height:'52vh'}}
        >Patiently Wait...we are registering you in</h4>:
